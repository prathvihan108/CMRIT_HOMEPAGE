  ...
  (function() {
    ...
    var runInContext = function runInContext2(context) {
      ...
      function get(object, path3, defaultValue) {
        var result2 = object == null ? undefined : baseGet(object, path3);
      ...
      function result(object, path3, defaultValue) {
        ...
        while (++index < length) {
          var value = object == null ? undefined : object[toKey(path3[index])];
          if (value === undefined) {
            index = length;
            value = defaultValue;
          }
      ...
      function propertyOf(object) {
        return function(path3) {
          return object == null ? undefined : baseGet(object, path3);
        };
      }
...
async function getHtmlContent(payload) {
  ...
  content = content.replace("</body>", `  <script type="module" src="${entryJsPath}"></script>
</body>`);
...
function findPageName(id, pageDir) {
  const pId = isWin32 ? id.replace(/\//g, "\\") : id;
  const pDir = isWin32 ? pageDir.replace(/\//g, "\\") : pageDir;
  const pathWithoutPageDir = (0, import_lodash.last)(_path2.default.dirname(pId).split(pDir)) || "";
...
var PREFIX = "src";
var isWin322 = require("os").platform() === "win32";
  ...
  return {
      ...
      return () => {
        server.middlewares.use(async (req, res, next) => {
          ...
          const pageName = (() => {
            ...
            return _optionalChain([url, 'access', _5 => _5.match, 'call', _6 => _6(new RegExp(`${options.pagesDir}/(.*)/`)), 'optionalAccess', _7 => _7[1]]) || "index";
          })();
          ...
          content = await _optionalChain([server, 'access', _8 => _8.transformIndexHtml, 'optionalCall', _9 => _9(url, content, req.originalUrl)]);
          res.end(content);
        });
      };
    },
    resolveId(id) {
      if (id.endsWith(".html")) {
        const isMPA = typeof config.build.rollupOptions.input !== "string" && Object.keys(config.build.rollupOptions.input || {}).length > 0;
        if (!isMPA) {
          return `${PREFIX}/${_path2.default.basename(id)}`;
        } else {
          const pageName = findPageName(id, options.pagesDir);
          if (pageName in config.build.rollupOptions.input) {
            return isWin322 ? id.replace(/\\/g, "/") : `${PREFIX}/${options.pagesDir.replace("src/", "")}/${pageName}/index.html`;
          }
        }
      }
      return null;
    ...
    load(id) {
      if (isWin322 ? id.startsWith(resolve("").replace(/\\/g, "/")) && id.endsWith(".html") : id.startsWith(PREFIX)) {
        const idNoPrefix = id.slice(PREFIX.length);
        const pageName = findPageName(id, options.pagesDir);
        const page = options.pages[pageName] || {};
        const templateOption = page.template;
        const templatePath = templateOption ? resolve(templateOption) : resolve("public/index.html");
        const isMPA = typeof _optionalChain([config, 'access', _10 => _10.build, 'optionalAccess', _11 => _11.rollupOptions, 'access', _12 => _12.input]) !== "string" && Object.keys(_optionalChain([config, 'access', _13 => _13.build, 'optionalAccess', _14 => _14.rollupOptions, 'access', _15 => _15.input]) || {}).length > 0;
    ...
    closeBundle() {
      const isMPA = typeof _optionalChain([config, 'access', _16 => _16.build, 'optionalAccess', _17 => _17.rollupOptions, 'access', _18 => _18.input]) !== "string" && Object.keys(_optionalChain([config, 'access', _19 => _19.build, 'optionalAccess', _20 => _20.rollupOptions, 'access', _21 => _21.input]) || {}).length > 0;
      if (!isMPA && command === "build") {
        const root = config.root || process.cwd();
        const dest = config.build && config.build.outDir || "dist";
        const resolve2 = (p) => _path2.default.resolve(root, p);
        _shelljs2.default.mv(resolve2(`${dest}/${PREFIX}/*.html`), resolve2(dest));
        _shelljs2.default.rm("-rf", resolve2(`${dest}/${PREFIX}`));
      }
    }
  };
}
